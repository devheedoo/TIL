...
핵심 파일인 `bin/www` 에서 서버를 생성한다.

### 6.3 미들웨어

미들웨어는 익스프레스의 핵심이다. 요청-응답 사이에 위치하기 때문에 붙여진 이름이다. 라우터, 에러 핸들러 등이 이에 속한다.

미들웨어는 `app.use()` 에 의해 순서대로 실행되기 때문에 효율적으로 순서를 배치해야 한다.

- morgan: 요청 로그 출력
- winston: 요청 로그를 데이터베이스나 파일에 기록
- body-parser: 요청 분석
- cookie-parser: 쿠키 분석
- static: 정적 파일 제공
- express-session: 세션 관리
- connect-flash: 일회성 경고 메세지

### 6.4 Router 객체로 라우팅 분리하기

next('route'); 를 호출하면 이후의 미들웨어들을 건너뛰고 다음 라우터로 이동한다.

URL에 `:name` 형식으로 변수를 사용할 수 있다.
